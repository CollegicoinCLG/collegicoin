...
*Setup instructions for a gitian build of Collegicoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Collegicoin
...
- [Building Collegicoin](#building-collegicoin)
...
Clone the git repositories for collegicoin and gitian and then checkout the collegicoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the collegicoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Collegicoin
----------------------

To build Collegicoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the collegicoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/collegicoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/CollegicoinCLG/collegicoin
...
For example:
```bash
URL=https://github.com/konez2k/collegicoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit collegicoin=${COMMIT} --url collegicoin=${URL} ../collegicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit collegicoin=${COMMIT} --url collegicoin=${URL} ../collegicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit collegicoin=${COMMIT} --url collegicoin=${URL} ../collegicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/collegicoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/collegicoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/collegicoin-build.assert
```
...
[collegicoin/gitian.sigs](https://github.com/CollegicoinCLG/gitian.sigs/) repository, or if that's not possible to create a pull
request.
