...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CollegicoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CollegicoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CollegicoinMiner started (POS=%s)\n", (fProofOfStake ? "true" : "false") );
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("collegicoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running CollegicoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
