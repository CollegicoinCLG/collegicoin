...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("collegicoin-wallet");
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Collegicoin with fAggressive=true to
    // get as much data as possible.
    // Rewrite collegicoind data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> collegicoindData;
    bool allOK = dbenv.Collegicoin(newFilename, true, collegicoindData);
    if (collegicoindData.empty()) {
        LogPrintf("Collegicoin(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Collegicoin(aggressive) found %u records\n", collegicoindData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, collegicoindData) {
        if (fOnlyKeys) {
